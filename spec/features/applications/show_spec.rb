...
RSpec.describe 'application show page' do
  before(:each) do
    ...
    @application_1 = Application.create(name: "Jose Molina", street_address: "345 Colfax Ave", city: "Denver", state: "CO", zip_code: "80203", description: "I want a cat", status: "In Progress")
  end
...
it "case insensitive" do
  ...
  it 'can add pet to application' do
    ...
    expect(current_path).to eq("/applications/#{@application_1.id}")
    expect(@application_1.pets.first).to eq(@simba)
  end
  ...
  it "can adopt pets and submit" do
    visit "/applications/#{@application_1.id}"

    fill_in "search", with: "Terry"
    ...
    expect(current_path).to eq("/applications/#{@application_1.id}")
    expect(@application_1.pets.count).to eq(1)
    expect(@application_1.pets).to eq(@simba)
